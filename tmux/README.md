...
If you're new to tmux, I recomm***REMOVED*** you read [tmux 2: Productive Mouse-Free
...
---------------
 ...
 - **I'm using Bash On Windows (WSL), colors and Powerline look are broken.**
   ...
   Also, until Window's console replaces its GDI based r***REMOVED***er with a DirectWrite
...
- `L` jumps to the ***REMOVED*** of line
...
- use your preferred font along with the [Powerline font][powerline font] (that
  only contains the Powerline symbols): [this highly dep***REMOVED***s on your operating
...
This configuration supports the following builtin variables:
 ...
 - `#{battery_bar***REMOVED***`: horizontal battery charge bar
 - `#{battery_percentage***REMOVED***`: battery percentage
 - `#{battery_status***REMOVED***`: is battery charging or discharging?
 - `#{battery_vbar***REMOVED***`: vertical battery charge bar
 - `#{circled_session_name***REMOVED***`: circled session number, up to 20
 - `#{hostname***REMOVED***`: SSH/Mosh aware hostname information
 - `#{hostname_ssh***REMOVED***`: SSH/Mosh aware hostname information, blank when not
   connected to a remote server through SSH/Mosh
 - `#{loadavg***REMOVED***`: load average
 - `#{pairing***REMOVED***`: is session attached to more than one client?
 - `#{prefix***REMOVED***`: is prefix being depressed?
 - `#{root***REMOVED***`: is current user root?
 - `#{synchronized***REMOVED***`: are the panes synchronized?
 - `#{uptime_y***REMOVED***`: uptime years
 - `#{uptime_d***REMOVED***`: uptime days, modulo 365 when `#{uptime_y***REMOVED***` is used
 - `#{uptime_h***REMOVED***`: uptime hours
 - `#{uptime_m***REMOVED***`: uptime minutes
 - `#{uptime_s***REMOVED***`: uptime seconds
 - `#{username***REMOVED***`: SSH/Mosh aware username information
 - `#{username_ssh***REMOVED***`: SSH aware username information, blank when not connected
...
[wttr.in]:
```
tmux_conf_theme_status_right='#{prefix***REMOVED***#{pairing***REMOVED***#{synchronized***REMOVED*** #(curl wttr.in?format=3) , %R , %d %b | #{username***REMOVED***#{root***REMOVED*** | #{hostname***REMOVED*** '
```
...
**I don't recomm***REMOVED*** running this configuration with Cygwin anymore. Forking
...
Particularly, Mintty's text r***REMOVED***ering is implemented with GDI which has
limitations:
